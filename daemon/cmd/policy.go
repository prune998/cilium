...
func (r *PolicyReactionEvent) Handle(res chan interface{}) {
	...
	r.wg.Wait()
	r.reactToRuleUpdates(r.epsToBumpRevision, r.endpointsToRegen, r.newRev, r.upsertPrefixes, r.releasePrefixes, r.uid)
}
...
func (r *PolicyReactionEvent) reactToRuleUpdates(epsToBumpRevision, epsToRegen *policy.EndpointSet, rev uint64, upsertPrefixes, releasePrefixes []*net.IPNet, uid k8sTypes.UID) {
	...
	for _, c := range releasePrefixes {
		r.d.ipcache.RemoveLabels(c.String(), cidr.GetCIDRLabels(c), source.CustomResource, uid)
	}
	...
	for _, c := range upsertPrefixes {
		...
		r.d.ipcache.UpsertLabels(c.String(), cidr.GetCIDRLabels(c), source.CustomResource, uid)
	}
}
...
func (h *deletePolicy) Handle(params DeletePolicyParams) middleware.Responder {
	...
	rev, err := d.PolicyDelete(lbls, &policy.DeleteOptions{Source: metrics.LabelEventSourceAPI})
	if err != nil {
