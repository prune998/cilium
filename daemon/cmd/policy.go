...
func (r *PolicyReactionEvent) Handle(res chan interface{}) {
	...
	r.wg.Wait()
	r.reactToRuleUpdates(r.epsToBumpRevision, r.endpointsToRegen, r.newRev, r.upsertIdentities, r.releasePrefixes)
}
...
func (r *PolicyReactionEvent) reactToRuleUpdates(epsToBumpRevision, epsToRegen *policy.EndpointSet, rev uint64, upsertIdentities map[string]*identity.Identity, releasePrefixes []*net.IPNet) {
	...
	if len(releasePrefixes) != 0 {
		r.d.ipcache.ReleaseCIDRIdentitiesByCIDR(releasePrefixes)
	}
	...
	if upsertIdentities != nil {
		r.d.ipcache.UpsertGeneratedIdentities(upsertIdentities)
	}
}
